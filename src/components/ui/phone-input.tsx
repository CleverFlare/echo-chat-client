    ...
    <StoreContext.Provider value={store}>
      <PhoneInputContext.Provider value={contextValue}>
        <RootPrimitive
          ...
          ref={composedRef}
          className={cn(
            "relative flex h-9 w-full items-center rounded-xl bg-input transition-[color,box-shadow] dark:bg-input/30 has-[[data-slot=phone-input-field]:focus-visible]:ring-3 has-[[data-slot=phone-input-field]:focus-visible]:ring-ring has-[[data-slot=phone-input-field][aria-invalid=true]]:border-destructive has-[[data-slot=phone-input-field][aria-invalid=true]]:ring-3 has-[[data-slot=phone-input-field][aria-invalid=true]]:ring-destructive/20 dark:has-[[data-slot=phone-input-field][aria-invalid=true]]:ring-destructive/40 data-disabled:cursor-not-allowed data-disabled:opacity-50",
            className,
          )}
        />
        {isFormControl && (
...
function PhoneInputCountrySelect(props: PhoneInputCountrySelectProps) {
  ...
  const onOpenChange = React.useCallback(
    (open: boolean, event: PopoverRoot.ChangeEventDetails) => {
      // ← add `event: Event`
      store.setState("open", open);
      onOpenChangeProp?.(open, event); // ← pass it through
    },
  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange} {...popoverProps}>
      <PopoverTrigger
        ...
        disabled={isDisabled}
        className={cn(
          "flex h-full shrink-0 items-center gap-2 rounded-l-xl border-r border-accent bg-transparent px-3 text-sm transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50",
          className,
        )}
      >
        {!countryContext ? (
      ...
      <PopoverContent className="w-[300px] p-0 dark" align="start">
        <Command>
...
function PhoneInputField(props: React.ComponentProps<"input">) {
  ...
  return (
    <Input
      ...
      ref={composedRef}
      className={cn(
        "h-full flex-1 rounded-l-none border-0 bg-transparent shadow-none focus-visible:ring-0 focus-visible:border-0 disabled:bg-transparent dark:bg-transparent dark:disabled:bg-transparent",
        className,
      )}
